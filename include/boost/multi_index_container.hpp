...
class multi_index_container
  ...
  , private detail::header_holder<
    ...
    >
  , public
      detail::multi_index_base_type<Value, IndexSpecifierList, Allocator>::type
{
private:
...
public:
  ...
  using ctor_args_list = typename super::ctor_args_list;
  using index_specifier_type_list = IndexSpecifierList;

  using index_type_list = typename super::index_type_list;

  using iterator_type_list       = typename super::iterator_type_list;
  using const_iterator_type_list = typename super::const_iterator_type_list;
  ...
  explicit multi_index_container(const ctor_args_list& args_list,
  ...
  template<typename InputIterator> multi_index_container(InputIterator first,
      InputIterator last,
    ...
    , node_count(0)
  {
    BOOST_MULTI_INDEX_CHECK_INVARIANT;
    try {
      using init_iterator = const Value*;
  ...
  multi_index_container(multi_index_container&& x, const allocator_type& al)
    : bfm_allocator(al)
    , bfm_header()
    , super(x, detail::do_not_copy_elements_tag())
    , node_count(0)
  ...
  multi_index_container& operator=(multi_index_container&& x) {
    if constexpr(
  ...
  multi_index_container& operator=(std::initializer_list<Value> list) {
    BOOST_MULTI_INDEX_CHECK_INVARIANT;
    using init_iterator = const Value*;
  ...
  struct index {
    using pos = mp11::mp_find_if_q<index_type_list, detail::has_tag<Tag>>;
  ...
  struct nth_index_iterator {
    using type = typename nth_index<N>::type::iterator;
  };

  template<int N>
  struct nth_index_const_iterator {
    using type = typename nth_index<N>::type::const_iterator;
  };
  ...
  struct index_iterator {
    using type = typename index<Tag>::type::iterator;
  };

  template<typename Tag>
  struct index_const_iterator {
    using type = typename index<Tag>::type::const_iterator;
  };

  template<typename Tag, typename IteratorType>
  typename index_iterator<Tag>::type project(IteratorType it) {
    using index_type = typename index<Tag>::type;
...
protected:
  using final_node_handle_type = typename super::final_node_handle_type;
  ...
  void copy_construct_from(const multi_index_container& x) {
    copy_map_type map(bfm_allocator::member, x.size(), x.header(), header());
    for (const_iterator it = x.begin(), it_end = x.end(); it != it_end; ++it)
  ...
  insert_(const Value& v, final_node_type* position, Variant variant) {
    final_node_type* x = 0;
  ...
  std::pair<final_node_type*, bool>
  insert_(const Value& v, final_node_type* position)
  { return insert_(v, position, detail::lvalue_tag()); }

  std::pair<final_node_type*, bool>
  insert_rv_(const Value& v, final_node_type* position)
  { return insert_(v, position, detail::rvalue_tag()); }

  template<typename T>
  std::pair<final_node_type*, bool>
  insert_ref_(T& t, final_node_type* position) {
    final_node_type* x = allocate_node();
    try {
      construct_value(x, t);
      try {
  ...
  std::pair<final_node_type*, bool>
  insert_ref_(const value_type& x, final_node_type* position)
  { return insert_(x, position); }

  std::pair<final_node_type*, bool>
  insert_ref_(value_type& x, final_node_type* position)
  { return insert_(x, position); }
  ...
  template<typename... Args>
  std::pair<final_node_type*, bool>
  emplace_hint_(final_node_type* position, Args&& ... args) {
    final_node_type* x = allocate_node();
    try {
      construct_value(x, std::forward<Args>(args)...);
      try {
  ...
  void swap_(multi_index_container& x) {
    swap_(x, std::bool_constant<
                node_alloc_traits::propagate_on_container_swap::value>());
  }

  void swap_(multi_index_container& x, std::true_type swap_allocators) {
  ...
  void swap_(multi_index_container& x, std::false_type swap_allocators) {
    std::swap(bfm_header::member, x.bfm_header::member);
    super::swap_(x, swap_allocators);
    std::swap(node_count, x.node_count);
  }

  void swap_elements_(multi_index_container& x) {
    std::swap(bfm_header::member, x.bfm_header::member);
  ...
  void save(Archive& ar, const unsigned int version) const {
    ...
    for (iterator it = super::begin(), it_end = super::end(); it != it_end;
         ++it)
    {
      serialization::save_construct_data_adl(ar, std::addressof(*it),
                                             value_version);
  ...
  bool invariant_() const { return super::invariant_(); }
...
get(multi_index_container<Value, IndexSpecifierList, Allocator>& m) noexcept {
  using multi_index_type = multi_index_container<
                              Value, IndexSpecifierList, Allocator>;
  using index_type = typename nth_index<multi_index_type, N>::type;

  static_assert(N >= 0
      && N < mp11::mp_size<typename multi_index_type::index_type_list>::value);
...
struct index {
  using index_type_list = typename MultiIndexContainer::index_type_list;
...
get(multi_index_container<Value, IndexSpecifierList, Allocator>& m) noexcept {
  using multi_index_type =
      multi_index_container<Value, IndexSpecifierList, Allocator>;
  using index_type =
      typename ::boost::multi_index::index<multi_index_type, Tag>::type;
...
template<typename MultiIndexContainer, int N>
struct nth_index_iterator {
  using type = typename nth_index<MultiIndexContainer, N>::type::iterator;
};

template<typename MultiIndexContainer, int N>
struct nth_index_const_iterator {
  using type = typename nth_index<MultiIndexContainer, N>::type::const_iterator;
};
        ...
        IteratorType it)
{
  ...
  static_assert(mp11::mp_contains<typename multi_index_type::iterator_type_list,
                                  IteratorType>::value);

  BOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(it);

#if defined(BOOST_MULTI_INDEX_ENABLE_SAFE_MODE)
  using converter = detail::converter<multi_index_type,
                                      typename IteratorType::container_type>;
...
typename nth_index_const_iterator<
  multi_index_container<Value, IndexSpecifierList, Allocator>, N>::type
project(const multi_index_container<Value, IndexSpecifierList, Allocator>& m,
        IteratorType it)
{
  using multi_index_type =
      multi_index_container<Value, IndexSpecifierList, Allocator>;
  ...
  static_assert(mp11::mp_contains<
        typename multi_index_type::iterator_type_list, IteratorType
      >::value
    || mp11::mp_contains<
...
template<typename MultiIndexContainer, typename Tag>
struct index_iterator {
  using type = typename ::boost::multi_index::index<MultiIndexContainer, Tag>
      ::type::iterator;
};
        ...
        IteratorType it)
{
  ...
  static_assert(mp11::mp_contains<typename multi_index_type::iterator_type_list,
                                  IteratorType>::value
      || mp11::mp_contains<typename multi_index_type::const_iterator_type_list,
                           IteratorType>::value);
...
#if defined(BOOST_MULTI_INDEX_ENABLE_SAFE_MODE)
  using converter =
    detail::converter<multi_index_type, typename IteratorType::container_type>;
  BOOST_MULTI_INDEX_CHECK_IS_OWNER(it, converter::index(m));
#endif
