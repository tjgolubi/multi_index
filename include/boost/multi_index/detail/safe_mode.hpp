...
// (http://www.horstmann.com/safestl.html).  In this mode, containers of type
// Container are derived from safe_container<Container>, and their corresponding
// iterators are wrapped with safe_iterator. These classes provide an internal
// record of which iterators are at a given moment associated to a given
// container, and properly mark the iterators as invalid when the container gets
// destroyed.  Iterators are chained in a single attached list, whose header is
// kept by the container. More elaborate data structures would yield better
// performance, but I decided to keep complexity to a minimum since speed is not
// an issue here.  Safe mode iterators automatically check that only proper
// operations are performed on them: for instance, an invalid iterator cannot be
// dereferenced. Additionally, a set of utilty macros and functions are provided
// that serve to implement preconditions and cooperate with the framework within
// the container.  Iterators can also be unchecked, i.e. they do not have info
// about which container they belong in. This situation arises when the iterator
// is restored from a serialization archive: only information on the node is
// available, and it is not possible to determine to which container the
// iterator is associated to. The only sensible policy is to assume unchecked
// iterators are valid, though this can certainly generate false positive safe
// mode checks.  This is not a full-fledged safe mode framework, and is only
// intended for use within the limits of Boost.MultiIndex.
...
// Checking routines. Assume the best for unchecked iterators (i.e. they pass
// the checking when there is not enough info to know.)
...
inline bool check_in_bounds(const Iterator& it, Difference n) {
  ...
  if (!it.valid())
    return false;
  return (n > 0) ? (it.owner()->end()   - it >= n)
                 : (it.owner()->begin() - it <= n);
}

template<typename Container>
inline
bool check_different_container(const Container& cont0, const Container& cont1)
{ return (&cont0 != &cont1); }

template<typename Container0, typename Container1>
inline
bool check_equal_allocators(const Container0& cont0, const Container1& cont1)
{ return (cont0.get_allocator() == cont1.get_allocator()); }

// Invalidates all iterators equivalent to that given. Safe containers must call
// this when deleting elements: the safe mode framework cannot perform this
// operation automatically without outside help.
...
inline void detach_equivalent_iterators(Iterator& it) {
  ...
  for (prev_=  static_cast<Iterator*>(&it.cont->header);
      (next_ = static_cast<Iterator*>(prev_->next)) != nullptr; )
  {
...
public:
  bool valid() const { return (cont != nullptr); }
...
protected:
  safe_iterator_base() : cont(), next(), unchecked_(false) { }
...
protected:
  ...
  ~safe_container_base() {
    // Detaches all remaining iterators, which by now will be those pointing to
    // the end of the container.

    for (safe_iterator_base* it = header.next; it; it = it->next)
      it->cont = nullptr;
    header.next = nullptr;
  }

  void swap(safe_container_base& x) {
    for (safe_iterator_base* it0 = header.next; it0; it0 = it0->next)
      it0->cont = &x;
    for (safe_iterator_base* it1 = x.header.next; it1; it1 = it1->next)
      it1->cont = this;
...
// * iterators must be generated via safe_iterator, which adapts a preexistent
// unsafe iterator class.
...
public:
  ...
  safe_iterator(const T0& t0, const T1& t1,
                safe_container<container_type>* cont_)
  ...
  const container_type* owner() const {
    return static_cast<const container_type*>(
...
// Serialization. Note that Iterator::save and Iterator:load are assumed to be
// defined and public: at first sight it seems like we could have resorted to
// the public serialization interface for doing the forwarding to the adapted
// iterator class:
// 
//  ar<<base_reference();
//  ar>>base_reference();
// 
// but this would cause incompatibilities if a saving program is in safe mode
// and the loading program is not, or viceversa --in safe mode, the archived
// iterator data is one layer deeper.  This is especially relevant with XML
// archives.  It'd be nice if Boost.Serialization provided some forwarding
...
class safe_container
  : public detail::safe_container_base
{
...
public:
  void detach_dereferenceable_iterators() {
    ...
    iterator* prev_, *next_;
    for (prev_ = static_cast<iterator*>(&this->header);
        (next_ = static_cast<iterator*>(prev_->next)) != nullptr; ) {
      if (*next_ != end_) {
        prev_->next = next_->next;
        next_->cont = nullptr;
  ...
  void swap(safe_container<Container>& x) { super::swap(x); }
}; // safe_container
