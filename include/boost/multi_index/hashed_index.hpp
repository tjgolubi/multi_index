...
protected:
  using index_node_type = hashed_index_node<typename super::index_node_type>;

private:
  using node_alg = typename index_node_type::template node_alg<Category>::type;
  using node_impl_type         = typename index_node_type::impl_type;
  using node_impl_pointer      = typename node_impl_type::pointer;
  using node_impl_base_pointer = typename node_impl_type::base_pointer;
  using bucket_array_type = bucket_array<typename super::final_allocator_type>;

public:
  /* types */

  using key_type       = typename KeyFromValue::result_type;
...
public:
  ...
  using ctor_args       = std::tuple<size_type, key_from_value, hasher,
                                     key_equal>;

#if defined(BOOST_MULTI_INDEX_ENABLE_SAFE_MODE)
  using iterator = safe_mode::safe_iterator<
                      hashed_index_iterator<index_node_type, bucket_array_type,
                          Category, hashed_index_global_iterator_tag>,
                      hashed_index
                   >;
#else
  using iterator = hashed_index_iterator<index_node_type, bucket_array_type,
                                    Category, hashed_index_global_iterator_tag>;
#endif

  using const_iterator = iterator;

  using local_iterator = hashed_index_iterator<index_node_type,
                                               bucket_array_type,
                                               Category,
...
protected:
  ...
  using index_type_list =
      mp11::mp_push_front<typename super::index_type_list, hashed_index>;
...
private:
#if defined(BOOST_MULTI_INDEX_ENABLE_SAFE_MODE)
  using safe_super = safe_mode::safe_container<hashed_index>;
#endif

  using value_param_type = typename detail::call_traits<value_type>::param_type;
  using key_param_type   = typename detail::call_traits<key_type>::param_type;
...
public:
  ...
  hashed_index& operator=(std::initializer_list<value_type> list) {
    this->final() = list;
    return *this;
  }
  ...
  iterator begin() noexcept {
    return make_iterator(index_node_type::from_impl(header()->next()->prior()));
  }
                ...
                const CompatibleHash& hash, const CompatiblePred& eq) const
  {
    return find(k, hash, eq,
                promotes_1st_arg<CompatiblePred, CompatibleKey, key_type>());
  }
  ...
  const_local_iterator end(size_type) const { return make_local_iterator(0); }
  ...
  void rehash(size_type n) {
    BOOST_MULTI_INDEX_HASHED_INDEX_CHECK_INVARIANT;
    if (size() <= max_load && n <= bucket_count())
      return;
...
protected:
  ...
  hashed_index(const hashed_index& x)
    : super(x)
  ...
  hashed_index(const hashed_index& x, do_not_copy_elements_tag)
  ...
  ~hashed_index() { } // The container is guaranteed to be empty by now.
  ...
  void copy_(const hashed_index& x, const copy_map_type& map)
  { copy_(x, map, Category()); }

  void copy_(const hashed_index& x, const copy_map_type& map, hashed_unique_tag)
  {
    if (x.size() != 0) {
      node_impl_pointer end_org = x.header()->impl();
      node_impl_pointer org = end_org;
      node_impl_pointer cpy = header()->impl();
      do {
        node_impl_pointer prev_org = org->prior();
        node_impl_pointer prev_cpy =
        ...
        if (node_alg::is_first_of_bucket(org)) {
          ...
          node_impl_base_pointer buc_cpy =
      ...
      do {
        ...
        if (node_alg::is_first_of_bucket(next_org)) {
          ...
          if (org->next() == node_impl_type::base_pointer_from(next_org)) {
            ...
            cpy->next() = node_impl_type::base_pointer_from(
                static_cast<index_node_type*>(map.find(
                    static_cast<final_node_type*>(
                        index_node_type::from_impl(
                          node_impl_type::pointer_from(
                            org->next())))))->impl());
          }

          if (next_org->prior() != org) {
            next_cpy->prior() = static_cast<index_node_type*>(
                map.find(static_cast<final_node_type*>(
  ...
  void delete_all_nodes_() { delete_all_nodes_(Category()); }

  void delete_all_nodes_(hashed_unique_tag) {
    for (node_impl_pointer x_end = header()->impl(), x = x_end->prior();
         x != x_end; ) {
  ...
  void swap_(hashed_index& x, BoolConstant swap_allocators) {
    adl_swap(key, x.key);
  ...
  void swap_elements_(hashed_index& x) {
  ...
  bool check_rollback_(index_node_type* x) const {
    std::size_t buc = find_bucket(x->value());
    return in_place(x->impl(), key(x->value()), buc)
          && super::check_rollback_(x);
  }
  ...
  bool equals(const hashed_index& x, hashed_non_unique_tag) const {
    ...
    for (const_iterator it = begin(), it_end = end(); it != it_end;) {
      ...
      const_iterator it_last = make_iterator(
                                  index_node_type::from_impl(end_of_range(
                                      it.get_node()->impl())));
      if (std::distance(it, it_last) != std::distance(it2, it2_last))
        return false;
      ...
      if (it != it_last) {
        for (const_iterator scan = it; scan != it_last; ++scan) {
          if (std::find(it, scan, *scan) != scan)
            continue;
          difference_type matches = std::count(it2, it2_last, *scan);
          if (matches == 0 || matches != std::count(scan, it_last, *scan))
            return false;
        }
        it = it_last;
      }
    ...
    if (size() == 0 || begin() == end()) {
      ...
      for (size_type buc = 0; buc < bucket_count(); ++buc) {
        ...
        for (const_local_iterator it = begin(buc), it_end = end(buc);
             it != it_end; ++it, ++ss1)
        {
          if (find_bucket(*it) != buc)
            return false;
        }
        if (ss1 != bucket_size(buc))
          return false;
        s1 += ss1;
      }
      if (s1 != size())
...
private:
  ...
  std::size_t find_bucket(value_param_type v) const { return bucket(key(v)); }
  ...
  using link_info = std::conditional_t<
                      std::is_same_v<Category, hashed_unique_tag>,
                      node_impl_base_pointer, link_info_non_unique
                    >;
  ...
  node_impl_pointer last_of_range(node_impl_pointer x, hashed_non_unique_tag)
      const
  {
  ...
  node_impl_pointer end_of_range(node_impl_pointer x, hashed_non_unique_tag)
      const
  {
    node_impl_base_pointer y = x->next();
    node_impl_pointer      z = y->prior();
  ...
  void link(index_node_type* x, const link_info_non_unique& pos,
            hashed_non_unique_tag)
  {
  ...
  void unlink(index_node_type* x) { node_alg::unlink(x->impl()); }

  using unlink_undo = typename node_alg::unlink_undo;
  ...
  void unchecked_rehash(size_type n, hashed_unique_tag) {
    ...
    if (size() != 0) {
      ...
      auto_space<node_impl_pointer, allocator_type>
         ...
         y != node_impl_pointer(0);)
    {
      if (node_alg::is_first_of_group(y)) { /* group of 3 or more */
        if (y == x) {
          ...
          return eq_(k, key(index_node_type::from_impl(
  ...
  template<typename CompatibleHash, typename CompatiblePred>
  iterator find(const key_type& k, const CompatibleHash& hash,
  ...
  template<typename CompatibleHash, typename CompatiblePred>
  size_type count(const key_type& k, const CompatibleHash& hash,
                  const CompatiblePred& eq, std::true_type) const
  ...
  template<typename CompatibleHash, typename CompatiblePred>
  std::pair<iterator, iterator> equal_range(
      const key_type& k, const CompatibleHash& hash, const CompatiblePred& eq,
      std::true_type) const
...
bool operator==(const hashed_index<K, H, P, S, T, C>& x,
                const hashed_index<K, H, P, S, T, C>& y)
{ return x.equals(y); }
...
template<typename K, typename H, typename P, typename S, typename T, typename C>
void swap(hashed_index<K, H, P, S, T, C>& x, hashed_index<K, H, P, S, T, C>& y)
{ x.swap(y); }
...
struct hashed_unique {
  using index_args = typename detail::hashed_index_args<Arg1, Arg2, Arg3, Arg4>;
  using tag_list_type = typename index_args::tag_list_type;
  using key_from_value_type = typename index_args::key_from_value_type;
  using hash_type = typename index_args::hash_type;
  using pred_type = typename index_args::pred_type;

  template<typename Super>
  struct node_class {
    using type = detail::hashed_index_node<Super>;
  };

  template<typename SuperMeta>
  struct index_class {
    using type = detail::hashed_index<key_from_value_type, hash_type, pred_type,
                         SuperMeta, tag_list_type, detail::hashed_unique_tag>;
  };
}; // hashed_unique
...
struct hashed_non_unique {
  ...
  struct index_class {
    using type = detail::hashed_index<key_from_value_type, hash_type, pred_type,
                      SuperMeta, tag_list_type, detail::hashed_non_unique_tag>;
  };
}; // hashed_non_unique
